// ==UserScript==
// @name         AADigital Turbo Hunter FINAL B - +201118568524
...
// Create floating note window (only once)
const noteBox = document.getElementById('noteBox');
const addNoteBtn = document.getElementById('addNoteBtn');
const clearNoteBtn = document.getElementById('clearNoteBtn');

// Create floating note window
const floatingNote = document.createElement('div');
floatingNote.id = 'floatingNote';
...
floatingNote.innerHTML = `
    <div id="floatingNoteText" style="margin-bottom: 20px;"></div>
    <button id="hideFloatingNoteBtn" style="
        padding: 8px 12px;
        background: #dc3545;
        color: #fff;
        border: none;
...
const floatingNoteText = document.getElementById('floatingNoteText');
const hideFloatingNoteBtn = document.getElementById('hideFloatingNoteBtn');

hideFloatingNoteBtn.addEventListener('click', () => {
    floatingNote.style.display = 'none';
});

// Load existing notes from localStorage
let savedNotes = JSON.parse(localStorage.getItem('floatingNotes')) || [];
...
function createNoteElement(noteText) {
    ...
    noteItem.addEventListener('click', () => {
        floatingNoteText.textContent = noteText;
        floatingNote.style.display = 'block';
    });
    return noteItem;
}

addNoteBtn.addEventListener('click', () => {
    const userNote = prompt("Enter your note:");
    if (userNote && userNote.trim() !== "") {
        savedNotes.push(userNote);
        localStorage.setItem('floatingNotes', JSON.stringify(savedNotes));
        renderNotes();
    }
});
...
clearNoteBtn.addEventListener('click', () => {
    savedNotes = [];
    localStorage.removeItem('floatingNotes');
    renderNotes();
});
